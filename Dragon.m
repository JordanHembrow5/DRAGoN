...
%   so will depend on your microscopy setup (in getParams.m)
    ...
    im_stack = grabImageStack(path + raw_stack_filename);
    im_raw = maxProjection(im_stack);
    imwrite(im_raw, path + projection_filename);
    im_raw = cytoFilter(path);
    

    % Grab 2D Network
    im_mask = grabCellMask(path+mask_filename, im_raw);
    ...
    save(path + variables_filename, 'net_props');    % Store all of the raw data in .mat format
...
function outDat(np, path)
    ...
    fprintf(datafile, "%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n", np.branchRatio, np.avgFilXY, np.avgFilZ, np.avgBranchXY, np.avgBranchZ, np.avgLen, np.Structures.mean, np.Structures.freq, np.skelDensity, np.cellBPDensity, mean(np.deviation, 'all', 'omitnan'), mean(np.curvature, 'all', 'omitnan'), mean(np.curvatureSigned,'all', 'omitnan'), mean(np.filWidth, 'all', 'omitnan'));
    fclose(datafile);            
end
