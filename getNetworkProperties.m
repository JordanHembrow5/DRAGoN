...
%   @output: np - A range of network properties in a structure
    ...
    np.cellSize = sum(im_mask2>0, 'all');
    np.skelDensity = sum(np.imSkel, 'all')/double(np.cellSize);
    np.cellBPDensity = sum(bwmorph(np.imSkel, 'branchpoints'), 'all')/double(np.cellSize);
    ...
    np.deviation = avgDeviation(np.skelLabel);
...
function d = avgDeviation(skel_label)
    no_of_filaments = max(skel_label, [], 'all');
    ...
    for f = 1:no_of_filaments
        ...
        if(size(ends,1) == 2)
            sum = 0.0; len = size(coords,1);
            for j = 1:len
                sum = sum + distToLine(ends, coords(j,:));
            end
            d(f) = sum / double(len);
        end
    end
